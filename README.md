...
When `useClaude` is enabled, `modify_private_settings.json` merges the following into `~/.claude/settings.json` (existing user settings are preserved):

| Setting | Value | Description |
|---|---|---|
| `autoUpdatesChannel` | `"latest"` | Auto-updates to each release as it ships; a leftover `env.DISABLE_AUTOUPDATER` is removed |
| `attribution` | `false` | Removes all commit and PR attribution (needs Claude Code 2.1.281+) |
